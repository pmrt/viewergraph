...
func TestStreamBatcherChatterSizeLowerThanMax(t *testing.T) {
	...
	b := &StreamBatcher{
		MaxQueueSize: 10,
		FlushFunc:    func(sto database.Storage, queue []string, channel string) error { return nil },
	}
...
func TestStreamBatcherJSONParse(t *testing.T) {
	t.Parallel()
	...
	if err := b.Batch(tee); err != nil {
		t.Fatal(err)
	}

	if got == nil {
		t.Fatal("expected got to not be empty")
	}

	var want ViewersJSON
	if err := json.Unmarshal(buf.Bytes(), &want); err != nil {
		if err != io.EOF {
			t.Fatal(err)
		}
	}
	if diff := deep.Equal(got[9:], want.Chatters.Viewers); diff != nil {
		t.Fatal(diff)
	}
}

func TestStreamBatcherJSONQueue(t *testing.T) {
	t.Parallel()
	...
	b := &StreamBatcher{
		MaxQueueSize: 100,
		FlushFunc: func(sto database.Storage, _ []string, _ string) error {
			flushCount++
			return nil
		},
	}

	if err := b.Batch(obj); err != nil {
		t.Fatal(err)
	}

	var got, want uint64

	got, want = flushCount, 13
	if got != want {
		t.Fatalf("flushCount - got: %d want: %d", got, want)
	}
	got, want = b.queueCount, 0
	if got != want {
		t.Fatalf("queueCount - got: %d want: %d", got, want)
	}
	got, want = b.ChatterSize, 1204
	if got != want {
		t.Fatalf("ChatterSize - got: %d want: %d", got, want)
	}
}
